...
orientation, then the relevant docs under [`docs/development/`](docs/development/) — especially
`principles.md`, `design.md`, and `roadmap.md`. A few invariants are non-negotiable and PRs that break them will be
declined:
...
- **The pipeline order in `docs/development/design.md` §4.2 is load-bearing.** Verbatim tokenizes first, text stages
...
the Metal Toolchain for the Qwen3-ASR engine, XcodeGen), the build variants, and a one-time self-signed cert so
...
- **File-based storage, no SQLite.** Persisted config carries a `schema_version` and migrates
  forward (`docs/development/design.md` §5.1).
- **Reuse the UI vocabulary** in `docs/development/ui_components.md` rather than inventing competing badges or
