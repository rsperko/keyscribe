...
term" hint — so a dictionary entry is never a complete no-op, on any engine.
...
| **Dictionary recovery** | post-STT | Snaps mangled words to dictionary terms ("charge bee" → "ChargeBee") via the `FuzzyStage` (Levenshtein + Soundex gated; the dictionary stays a *hint*, not a protected substitution). **Runs unconditionally whenever the mode's merged dictionary is non-empty** — no toggle: adding a term is the opt-in, removing it the remedy, per-mode dictionaries scope it (matching Replacements). This reaches **every** engine's output, not only the bias-capable ones. The one mechanism control that remains is the per-engine recognition-bias **disable** (`recognition_bias_disabled_engines`); the former per-engine recovery keys (`dictionary_recovery_enabled_engines` / `dictionary_recovery_disabled_engines`) are obsolete — ignored on read, dropped on write. |
| **Screen-term recovery** | post-STT, **after** dictionary recovery | Identifiers visible in the captured preceding text (`ScreenTermExtractor`: camelCase, snake_case, letter+digit, dotted-name components — never plain prose words) become **ephemeral, per-dictation** terms for a second `FuzzyStage`. Rides the mode's **preceding-text opt-in** with no separate flag, mirroring dictionary recovery; privacy mode and a secure field both fail closed upstream (they empty `effectiveContext`, so the probe never runs). Matching is **exact-normalized AND single-token** (`FuzzyCorrector.Matching.exactNormalized`): it may re-case one token the user already said (`usestate` → `useState`) but never substitutes a different word and never joins across a token boundary — with `useState` on screen, "use state funds" stays untouched. Multi-token joins are **not** recovered: feeding them to the LLM as `fuzzyCandidates` was built, measured, and **rejected** — a 2-token join is indistinguishable from ordinary prose whose words happen to concatenate into a visible identifier ("use state funds" vs `useState`), and models split both ways on it (evals/rewrite/README.md). The curated **Dictionary** is the consented form of that feature. The local harvest never awaits — it reads the value the probe task published, so NO dictation path is charged for the field read, and a probe landing after commit simply yields no snap. Sorted **last** (order 40) so a curated dictionary term always wins, and the extractor drops any harvest colliding with one. Harvested terms are never persisted as config and never logged (count only) — but on a rewrite they render into the prompt, which IS stored in history like any other prompt. |
| **Verbatim** | tokenize **before** the text stages / restore last | A **live edit** (enabled by the live-edits toggle): spans delimited by spoken triggers ("begin verbatim" / "end verbatim") are pulled into a **single nonce token** **before** live edits / replacements / numbers / fuzzy run, so the span is protected from **everything except STT** (the text stages and the LLM all see only the token); restored verbatim after. Same machinery as redaction, different intent and position (protect-from-edit, first vs withhold-sensitive, last). **Spans are flat** (no nesting): the first "begin verbatim" pairs with the first close marker; a second begin before that close is protected as content, and a lone "end verbatim" with no open begin is ordinary prose. **Close matching is exact-first, then a phonetic "end"-like rescue:** an exact "end verbatim" always wins; only if a span has no exact end does a fallback pass accept a close where the word before "verbatim" is phonetically "end" (`FuzzyCorrector.phoneticKey` prefix + `levenshtein`, so the STT "and"↔"end" and "en"↔"end" mishears close the span while "send/bend/lend verbatim" do not) — the rescue can never override a correctly-closed span. **An unclosed span fails closed**: it protects to the end of the utterance (so content the user meant to shield never reaches the LLM) **and keeps the literal "begin verbatim" marker as the visible tell** — tokenized *with* the content, so it survives an LLM rewrite (the gate only guarantees nonce tokens, not surrounding prose) — making a missed close legible in the inserted text rather than a silent raw tail; one ⌘Z still removes the whole dictation. |
...
3. verbatim mark  verbatim tokenize · then clipboard tokenize   (apply) — BEFORE the text stages, so
                                            ...
                                            span stays literal
4. post-STT text  live edits → replacements → numbers (ITN) → dictionary recovery →
                  screen-term recovery   (apply)
                  (StageOrder: liveEdits 0 · replacements 10 · numbers 20 · fuzzy 30 · screenTerms 40;
                   dictionary recovery is the FuzzyStage, running whenever the mode's merged
                   dictionary is non-empty — NOT gated by the engine's bias capability;
                   screen-term recovery is a second FuzzyStage over identifiers harvested from the
                   captured preceding text, sorted last so a curated dictionary term always wins)
...
- **A whole-utterance replacement short-circuits the rest of the pipeline** — when one replacement
  ...
  is the winning rule's generated output. Bypassing the LLM makes redaction moot — nothing leaves the
  machine.
