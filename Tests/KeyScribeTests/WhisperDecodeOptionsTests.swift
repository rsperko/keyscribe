...
// "" and surface as a false "No speech detected". Batch push-to-talk dictation must never gamble the
// whole utterance on first-token confidence; these tests pin that the engine builds its options with
...
// ("And does it work this time?" spoken in 0.92 s reproduced it every run). The engine pads short
// audio past that guard with trailing zeros — indistinguishable from the zero-fill WhisperKit itself
// applies to reach the 30 s window — and leaves longer audio untouched.
