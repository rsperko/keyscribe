...
public enum FragmentStore {
    ...
    /// Resolve `name` to a fragment id, creating `dir/<id>.md` with a starter header+body when no
    /// fragment with that id exists. An existing file is left untouched so referencing it by name
    /// never clobbers its content. Returns the id. Throws on a name with no usable characters.
    @discardableResult
