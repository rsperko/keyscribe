...
// saved to <dir> before the pipeline deletes it (the WAV normally lives only record→transcribe→delete).
// Enables offline analysis (glitch/SINAD scoring, waveform inspection) and lets `--capture-probe` keep
// its recordings.
enum CaptureArchive {
    ...
    // No-op unless the env var is set. `tag` prefixes the saved name so commit vs probe captures are
